...
import com.goldenwise.intellstockpicker.domain.EMAState;
import com.goldenwise.intellstockpicker.domain.FunData;
...
public class MathOperatorService {
	...
	public static Observable<Pair<DateTime, Double>> MAOSC(Observable<Pair<DateTime, Double>> close, int N1, int N2) {
		Observable<Pair<DateTime, Double>> sma1 = MathOperatorService.SMA(close, N1).skip(N2 - N1);
		Observable<Pair<DateTime, Double>> sma2 = MathOperatorService.SMA(close, N2);
	...
	public static Observable<Pair<DateTime, Double>> GOC(Observable<Pair<DateTime, Double>> high,
		...
		Observable<Pair<DateTime, Double>> hoh_N1 = MathOperatorService.HOH(high, N1);
		Observable<Pair<DateTime, Double>> hoh_N2 = MathOperatorService.HOH(high, N2);
		Observable<Pair<DateTime, Double>> lol_N1 = MathOperatorService.LOL(low, N1);
		Observable<Pair<DateTime, Double>> lol_N2 = MathOperatorService.LOL(low, N2);
	...
	public static Observable<Pair<DateTime, Double>> CMCI(Observable<Pair<DateTime, Double>> tp, int N,
		...
		Observable<Pair<DateTime, Double>> meanDeviation = MathOperatorService.MeanDeviation(tp, N);

		Observable<Pair<DateTime, Double>> avg = MathOperatorService.SMA(tp, N);
	...
	public static Observable<Pair<DateTime, Double>> TR(Observable<Pair<DateTime, Double>> high,
		...
		Observable<Triple<DateTime, Double, Double>> close_v = MathOperatorService.SHIFT(close);
	...
	public static Observable<Pair<DateTime, Double>> DMI(Observable<Pair<DateTime, Double>> high,
		...
		// 1.upmove
		Observable<Pair<DateTime, Double>> upMove = MathOperatorService.DIFF(high);

		// 2.DownMove
		Observable<Pair<DateTime, Double>> downMove = MathOperatorService.DIFF(low);

		Observable<Pair<DateTime, Double>> plusDM = Observable.zip(upMove, downMove,
				(u, d) -> (new ImmutablePair(u.getLeft(),
						MathOperatorService.compareUpMoveAndDownMove(u.getRight(), d.getRight()))));

		Observable<Pair<DateTime, Double>> minusDM = Observable.zip(upMove, downMove,
				(u, d) -> (new ImmutablePair(u.getLeft(),
						MathOperatorService.compareUpMoveAndDownMove(d.getRight(), u.getRight()))));

		Observable<Pair<DateTime, Double>> tr = MathOperatorService.TR(high, low, close);

		Observable<Double> tr_v = tr.map(x -> x.getRight());
		Observable<DateTime> tr_t = tr.skip(N - 1).map(x -> x.getLeft());
		double initial_v = MathOperatorService.SMA(tr, N).first().map(x -> x.getRight()).toBlocking().last();
		...
		Observable<Pair<DateTime, Double>> plusADM = MathOperatorService.SMA(plusDM, N);

		Observable<Pair<DateTime, Double>> minusADM = MathOperatorService.SMA(minusDM, N);
		...
		double initial_dx = MathOperatorService.SMA(dx, N).first().map(x -> x.getRight()).toBlocking().last();
	...
	public static Observable<Pair<DateTime, Double>> TMA(Observable<Pair<DateTime, Double>> close, int N) {
		Observable<Pair<DateTime, Double>> sum = MathOperatorService.SMA(close, N);
		for (int i = 1; i < N; i++) {
			sum.zipWith(MathOperatorService.SMA(close, N - i).skip(i),
	...
	public static Observable<Pair<DateTime, Double>> REX(Observable<Pair<DateTime, Double>> high,
		...
		return MathOperatorService.EMA(MathOperatorService.SMA(tvb, N1), N2);
	}
